...
class LinearRegression {
    ...
    gradientDescent(features, labels){
       ...
       // where xi is parameter and Actual is the actual label value for that parameter
        const currentGuesses = features.matMul(this.weights); // Matrix multiplication in Tensor Flow
    ...
    train(){
        ...
        for(let i=0; i<this.options.iterations; i++){
            for(let j=0;j<batchQuantity; j++){
                ...
                const featureSlice = this.features.slice([startIndex, 0], [batchSize, -1]); // -1 means full (row/colum)
                const labelSlice = this.labels.slice([startIndex, 0], [batchSize, -1])
                this.gradientDescent(featureSlice, labelSlice);
            }
            this.bHistory.push(Array.from(this.weights.dataSync())[0]);
    ...
    recordMSE(){
        ...
        // console.log('=========MSE===', MSE);
        this.mseHistory.unshift(mse_val[0]);
    }
